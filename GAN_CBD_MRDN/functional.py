...
def fft_reshape(H_shape, img_height, img_width):
    x = len(H_shape)    # 2048  32*64
    y = len(H_shape[0])    # n  样本数
    ...
    H_s = np.zeros([x,y], dtype=complex)    
    for ii in range(x):
        for iii in range(y):
            H_s[ii,iii] = H_shape[ii,iii][0] + 1j*H_shape[ii,iii][1]
    for i in range(y):
        H_reshape[i, :] = np.reshape(H_s[:, i], (img_width, img_height))
        H_reshape_T[i, :] = H_reshape[i, :].T
...
def add_noise1(input, SNR):
    ...
    SNR_level = 10**(SNR/10)
    noise_level = power_level_1/SNR_level
    n_l = math.sqrt(noise_level)                                                        #sigma
    Noise_map = np.zeros([64, 1], dtype=complex)                                       #64 32
    Noise_map_real = np.zeros([64, 1], dtype=complex)                                  #64 32
    Noise_map_imag = np.zeros([64, 1], dtype=complex)                                  #64 32
    Noise_map_real = n_l * math.sqrt(1 / 2) * np.random.randn(64, 1)                  #0~1的数据
    Noise_map_imag = 1j * n_l * math.sqrt(1 / 2) * np.random.randn(64, 1)
    Noise_map = Noise_map_real + Noise_map_imag
    return Noise_map, n_l

def add_noise2(input, SNR):
    #x_test_realc = np.reshape(input, (1, -1))
    #power = np.sum(abs(x_test_realc) ** 2, axis=1)
    #power_level = power
    #power_level_1 = power_level/2048
    ...
    # noise_level = power_level_1/SNR_level
    n_l = math.sqrt(noise_level)                                                        #sigma
    Noise_map = np.zeros([128, 1], dtype=complex)                                       #64 32
    Noise_map_real = np.zeros([128, 1], dtype=complex)                                  #64 32
    Noise_map_imag = np.zeros([128, 1], dtype=complex)                                  #64 32
    Noise_map_real = n_l * math.sqrt(1 / 2) * np.random.randn(128, 1)                  #0~1的数据
    Noise_map_imag = 1j * n_l * math.sqrt(1 / 2) * np.random.randn(128, 1)
    Noise_map = Noise_map_real + Noise_map_imag
    return Noise_map, n_l
...
def compute_NMSE_linear(H_hat, H):
    H1 = np.reshape(H_hat, (len(H_hat), -1))
    H_pre1 = np.reshape(H, (len(H), -1))
    power = np.sum(abs(H_pre1) ** 2, axis=1)
    mse = np.sum(abs(  H_pre1 - H1) ** 2, axis=1)
...
def real_imag_stack(H):
    x = len(H)    # n
    H_real = np.zeros([x, 128, 1])           #32
    H_imag = np.zeros([x, 128, 1])
    H_out = np.zeros([x, 128, 2])
    for i in range(x):
        H_real[i, :] = H[i, :].real
        H_imag[i, :] = H[i, :].imag
...
def real_imag_stack_polar(H):
    x = len(H)    # n
    s = H.shape[1]
    H_real = np.zeros([x, s, 1])           #32
    H_imag = np.zeros([x, s, 1])
    H_out = np.zeros([x, s, 2])
    for i in range(x):
        H_real[i, :] = H[i, :].real
        H_imag[i, :] = H[i, :].imag
        H_out[i, :] = np.hstack((H_real[i, :], H_imag[i, :]))
...
def tensor_reshape(input1):
    ...
    H = torch.zeros([x, 1, s, 2])                      #64   64
    for i in range(x):
        H[i, 0, :, :] = input[i, 0:s, 0:2]
    return H
...
def ifft_tensor(H_shape):
    x = len(H_shape)    # n
    H_reshape_fft = np.zeros([x, 128, 1], dtype=complex)                #64 32
...
def ifft_tensor_polar(H_shape , Polarcodebook):
    x = len(H_shape)    # n
    H_reshape_fft = np.zeros([x, 128, 1], dtype=complex)                #64 32
    #H_real = np.zeros([x, 128, 1])
    #H_imag = np.zeros([x, 128, 1])
    #H_get = np.zeros([x, 128, 2])
    for i in range(x):
        H_reshape_fft[i, :, :] =  np.dot( Polarcodebook, H_shape[i, :, :])
        # H_reshape_fft[i, :, :] = ifft2(H_shape[i, :, :])
        # H_real[i, :] = H_reshape_fft[i, :].real
        # H_imag[i, :] = H_reshape_fft[i, :].imag
        # H_get[i, :] = np.hstack((H_real[i, :], H_imag[i, :]))
...
def ifft_tensor_polar_1(H_shape , Polarcodebook , SS):
    x = len(H_shape)    # n
    H_fft = np.zeros([x, SS, 1], dtype=complex)                #64 32
    for i in range(x):
        H_fft[i, :, :] = ifft2(H_shape[i, :, :])

    H_reshape_fft = np.zeros([x, 128, 1], dtype=complex)                #64 32
    for i in range(x):
        H_reshape_fft[i, :, :] =  np.dot( Polarcodebook, H_fft[i, :, :])
        
    return H_reshape_fft

def ifft_tensor_polar_2(H_shape , Polarcodebook , SS):
    x = len(H_shape)    # n
    ...
    H_fft = np.zeros([x, 128, 1], dtype=complex)                #64 32
    for i in range(x):
        H_fft[i, :, :] = ifft2(H_reshape_fft[i, :, :])
...
def randkaczmarz(A, b, iter):
    # Ax = b
    m = A.shape[0]
    n = A.shape[1]
    x = np.zeros([n,1],dtype=complex)
    normrow = np.zeros([1,m], dtype=float)
    index = np.zeros([1,m], dtype=int)
    row = np.zeros([1,n], dtype=complex)

    for i in range(m):
        normrow[:,i]=  np.linalg.norm( A[i,:], ord=2)  
        index[:,i] = i  
    
    weight = normrow / np.sum(normrow)
    # print(index ,type(index))

    for ii in range(iter):
        pickedi = np.random.choice(index.flatten(), size=1, replace=False, p=weight.flatten() )
        pickedi = pickedi[0]
        row[0,:]  = A[pickedi, :]
        a = ( b[pickedi] - np.dot(row , x) )
        aa =abs( (np.dot(row , np.transpose(row).conjugate() ) ) )
        aaa = a / aa
        x = x + np.multiply( aaa , np.transpose(row).conjugate() )        
    return x
...
def polar_reshape(A):
    m = A.shape[0]
    n = A.shape[1]   
    A_reshape = np.zeros([m,n], dtype=complex)
    for i in range(m):
        for ii in range(n):
            A_reshape[i,ii] = A[i,ii][0] + 1j* A[i,ii][1]
    return A_reshape


def polar_transform(Polarcodebook , H, H_n):
    ...
    S[0:128,0:128] =  ss
    # x =  np.dot( np.transpose(vv) , S ) 
    # x2 = np.transpose(uu).conjugate()
...
def polar_transform(Polarcodebook , H, H_n):
    ...
    ss = np.diag(ss)
    S = np.zeros([SS, 128], dtype=float) 
    S[0:128,0:128] =  ss[0:128 , 0:128]
    x =  np.dot( np.transpose(vv) , S ) 

    # x2 = np.transpose(uu).conjugate()

    x1 =  np.dot( x , np.transpose(uu) ) 
    H_p =   np.dot( x1.conjugate() , H ) 
    H_np = np.dot( x1.conjugate() , H_n )
    return H_p, H_np
'''